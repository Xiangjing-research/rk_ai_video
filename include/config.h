...
public:
    config(std::map<std::string, std::string> &mapConfigInfo)
    {
        use_hw = mapConfigInfo["use_hw"] == "True";
        url = mapConfigInfo["url"];
        fps = atoi(mapConfigInfo["fps"].c_str());
        preset = mapConfigInfo["preset"];
        tune = mapConfigInfo["tune"];
        profile = mapConfigInfo["profile"];
        protocol = mapConfigInfo["protocol"];
        capture_name = mapConfigInfo["capture_name"];
        trans_protocol = mapConfigInfo["trans_protocol"];
        width = atoi(mapConfigInfo["width"].c_str());
        height = atoi(mapConfigInfo["height"].c_str());
        codec_name = mapConfigInfo["codec_name"];
        model_name = mapConfigInfo["model_name"];
        input_path = mapConfigInfo["input_path"];

    };
    config(const std::string url, const std::string codec_name) : url(url),
    ...
    void set_model_name(const char *mn) { model_name = std::string(mn); };
    void set_input_path(const char *ip) { input_path = std::string(ip); };
    ...
    const char *get_model_name() { return model_name.c_str(); }
    const char *get_input_path() { return input_path.c_str(); }


    void print_config_str()
    {
        ...
        std::cout << " width: " << model_name << " height: " << height << std::endl;
        std::cout << " width: " << input_path << " height: " << height << std::endl;
    }
};
